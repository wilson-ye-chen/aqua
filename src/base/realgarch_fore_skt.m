function ...
    ...
    realgarch_fore_skt(D, r, logX, nEst, intEst, iStart, iEnd)
% [DFore, var1, var5, score1, score5, Theta, AccRate, Mapc] = ...
% realgarch_fore_skt(D, r, logX, nEst, intEst, iStart, iEnd) computes
% one-step-ahead Bayesian forecasts using the Realised-GARCH-skt model.
%
% Input:
...
% Theta   - posterior mean of Realised-GARCH-skt parameters.
...
% Date:   November 20, 2015
    ...
    [Chain, Accept, pc] = realgarch_est_skt(rEst, logXEst, nIter, nDisc);
    ...
    for i = 1:nFore
        ...
        QAve = realgarch_aveq_skt(u, Chain, logXFore, logSigmaSq0);
        var1(i) = QAve(end, 1);
        var5(i) = QAve(end, 2);
        ...
        if (mod(i, intEst) == 0) && (i ~= nFore)
            ...
            [Chain, Accept, pc] = realgarch_est_skt( ...
