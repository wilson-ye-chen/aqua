function ...
    ...
    aqua_fore_tc(D, Xi, r, nEst, intEst, iStart, iEnd)
% [DFore, Q, var1, var5, score1, score5, s1, s5, Mn, Sd, w, Theta, ...
% AccRate, Mapc, Chain] = aqua_fore_tc(D, Xi, r, nEst, intEst, iStart, iEnd)
% computes one-step-ahead Bayesian forecasts using the AQUA-gh-tc model.
%
% Input:
...
% s1      - posterior mean of quantile regression coefficient at 0.01 level.
% s5      - posterior mean of quantile regression coefficient at 0.05 level.
...
% Date:   December 9, 2015
    ...
    x1 = QAve(1:(end - 1), 1);
    x5 = QAve(1:(end - 1), 2);
    chainS1 = qreg_al_est(rEst, x1, 0.01, nIterQreg, nDiscQreg);
    chainS5 = qreg_al_est(rEst, x5, 0.05, nIterQreg, nDiscQreg);
    ...
    for i = 1:nFore
        ...
        % Forecast daily VaRs
        var1(i) = qreg_aveqtl(chainS1, QAve(end, 1));
        var5(i) = qreg_aveqtl(chainS5, QAve(end, 2));
